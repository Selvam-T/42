...
int	display_exp_list(char **arr)
{
	...
	while (ptr[i] != NULL)
	{
		value = ft_substr_idx(ptr[i], '=', &index);// strdup() and free below
		key = ft_substr(ptr[i], 0, index + 1);
		if (index == (int)ft_strlen(ptr[i]))
			printf("declare -x %s\n", ptr[i]);
		else
			printf("declare -x %s\"%s\"\n", key, value);
		free_str(&key);//if I strdup value, I could free value here
		i++;
	}
	return (0);
}
...
int	export_err_handle(int mode, char *str)
{
	if (mode == -1)
		printf("bash: export: `%s': not a valid identifier\n", str);
	else if (mode == -2)
		printf("bash: export: invalid value \n");
	return (1);
}
...
int	execute_export(t_minishell *ms, char **args)
{
	...
	while (args[++i] != NULL)
	{
		...
		if (mode == 0 || mode == 1)
			...
			exit_status = export_err_handle(mode, pair[0]);
		free_doubleptr(pair);
	}
	return (exit_status);
}
